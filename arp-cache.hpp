...
/*
  ...
  The ARP cache entries hold IP->MAC mappings and are timed out every SR_ARPCACHE_TO seconds.
   ...
   until we send 5 ARP requests), you must fill out the following
   function that is called every second and is defined in arp-cache.cpp:
...
public:
  ...
  void
  handleRequest(std::shared_ptr<ArpRequest> request);
  ...
  queueArpRequest(uint32_t ip, const Buffer& packet, const std::string& iface);
   ...
   */
  void
  removeArpRequest(const std::shared_ptr<ArpRequest>& entry);
